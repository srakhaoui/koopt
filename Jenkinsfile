...
node {
    ...
    sh '''
        ...
        #Stop running task(s)
        aws ecs list-tasks --region ${REGION} --cluster ${CLUSTER} --service ${SERVICE_NAME} | jq .taskArns[] | tr -d '"' | while read line ; do aws ecs stop-task --region ${REGION} --cluster ${CLUSTER} --task ${line} ; done
